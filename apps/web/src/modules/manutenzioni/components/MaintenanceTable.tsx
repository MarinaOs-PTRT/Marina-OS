...
export function MaintenanceTable({ data }: Props) {
  if (data.length === 0) {
    return <div style={{ padding: 'var(--space-xl)', textAlign: 'center', color: 'var(--text3)' }}>Nessun lavoro trovato con i filtri selezionati.</div>
  }
      ...
      <tbody>
        {data.map(m => (
          <tr key={m.id}>
            ...
            <td style={{ textTransform: 'capitalize' }}>{m.origine}</td>
            <td>{m.assegnatoA}</td>
